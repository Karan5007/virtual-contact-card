...
cluster = Cluster(['10.11.12.5', '10.11.12.8'])
...
userids = [ "userid"+str(i) for i in range(100) ]
...
'''
rows = session.execute("SELECT userid, year, event FROM calendar WHERE userid='userid7'")
for row in rows:
    print row.userid, row.year, row.event
'''
